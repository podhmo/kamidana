...
def test_default_is_jinja2_strict_undefined(
    loader: TemplateLoader, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # unlike jinja2 itself, kamidana defaults to StrictUndefined: using a
    # variable that was not passed raises UndefinedError (#85)
    from kamidana.driver import _make_environment
...
def test_driver_is_strict_undefined_by_default(
    loader: TemplateLoader, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_cli_undefined_options(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    # strict is the default since #85; --no-strict-undefined opts out,
    # --strict-undefined remains accepted for backward compatibility
    ...
    # default: fails with a gentle error showing the location
    monkeypatch.setattr(
    ...
    # --no-strict-undefined: the missing variable renders as empty
    monkeypatch.setattr(
        sys,
        "argv",
        ["kamidana", "-d", "data.yaml", "--no-strict-undefined", "./t.j2"],
    )
    main()
    assert capsys.readouterr().out == "name: foo\nage: \n"

    # --strict-undefined: same failure as the default
    monkeypatch.setattr(
        sys,
        "argv",
...
def test_cli_batch_undefined_options(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    # default: fails with a gentle error
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kamidana-batch", "--no-strict-undefined", "batch.yaml"],
    )
    main()
